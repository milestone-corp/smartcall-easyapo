#!/bin/bash
#
# EasyApo RPA beans-shika 本番デプロイスクリプト (RPA04)
#
# 使用方法:
...
TEMP_DIR="/tmp"
TARBALL_NAME="smartcall-easyapo-beans-shika.tar.gz"
CONTAINER_NAME="beans-shika"
COMPOSE_FILE="docker-compose.beans-shika.yml"
SERVICE_PORT="3011"

echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}EasyApo RPA beans-shika デプロイスクリプト${NC}"
echo -e "${BLUE}========================================${NC}"
...
if echo "$HEALTH_RESULT" | grep -q '"status":"ok"'; then
    echo -e "${GREEN}ヘルスチェック (beans-shika:$SERVICE_PORT): OK${NC}"
    echo "$HEALTH_RESULT" | jq . 2>/dev/null || echo "$HEALTH_RESULT"
else
    echo -e "${RED}ヘルスチェック (beans-shika:$SERVICE_PORT): NG${NC}"
    echo "$HEALTH_RESULT"
    echo -e "\nログを確認してください:"
