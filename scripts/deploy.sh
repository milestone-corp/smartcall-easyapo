...
# scripts/deploy.sh
#
# 統一デプロイスクリプト（店舗ID指定 or staging）
#
# 使い方:
#   ./scripts/deploy.sh staging          # Staging環境 (RPA03) にデプロイ
...
#
# 前提:
#   - envs/<SHOP_ID>-<NAME>.env または envs/staging.env が存在すること
...
#   4. 踏み台 → RPAサーバへ転送
#   5. RPAサーバで展開、docker-compose.shop.yml で build & up
#   6. ヘルスチェック
#
...
if [ -z "$TARGET" ] || [[ "$TARGET" == --* ]]; then
    echo -e "${RED}使い方: ./scripts/deploy.sh <staging|店舗ID> [--dry-run]${NC}"
    echo ""
    echo "利用可能な env:"
    ls "$LOCAL_PATH/envs/" 2>/dev/null | grep -v '_template\|.gitkeep' | sed 's/^/  - /'
    exit 1
fi
