#!/bin/bash
#
# EasyApo RPA yuki-dental 本番デプロイスクリプト (RPA04)
#
# 使用方法:
...
#   - ステージングサーバー（153.126.214.207）経由でRPA04(192.168.20.73)にアクセスできること
#
# 処理内容:
#   1. ローカルリポジトリの状態確認
#   2. ソースコードをtarballに圧縮
#   3. ステージングサーバー経由でRPA04に転送
#   4. RPA04でファイルを展開
#   5. Dockerイメージをビルド・コンテナ再起動
#
...
RPA_SERVER="centos@192.168.20.73"
RPA_REMOTE_PATH="/home/centos/smartcall-easyapo"
SSH_KEY="$HOME/.ssh/milestone"
TEMP_DIR="/tmp"
TARBALL_NAME="smartcall-easyapo-yuki-dental.tar.gz"
CONTAINER_NAME="yuki-dental"
COMPOSE_FILE="docker-compose.yuki-dental.yml"
SERVICE_PORT="3012"

echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}EasyApo RPA yuki-dental デプロイスクリプト${NC}"
echo -e "${BLUE}========================================${NC}"
echo -e "ローカルパス: $LOCAL_PATH"
echo -e "リモートパス: $RPA_REMOTE_PATH"
echo -e "ターゲット:   $RPA_SERVER (RPA04)"
...
# 5. ステージングサーバーからRPA04に転送・展開
echo -e "\n${YELLOW}[5/6] RPA04に転送・展開${NC}"
...
if echo "$HEALTH_RESULT" | grep -q '"status":"ok"'; then
    echo -e "${GREEN}ヘルスチェック (yuki-dental:$SERVICE_PORT): OK${NC}"
    echo "$HEALTH_RESULT" | jq . 2>/dev/null || echo "$HEALTH_RESULT"
else
    echo -e "${RED}ヘルスチェック (yuki-dental:$SERVICE_PORT): NG${NC}"
    echo "$HEALTH_RESULT"
    echo -e "\nログを確認してください:"
...
echo -e "URL: http://192.168.20.73:$SERVICE_PORT"
echo -e "\nログを確認する場合:"
echo -e "  ssh -i ~/.ssh/milestone centos@153.126.214.207 \"ssh -i ~/.ssh/milestone $RPA_SERVER 'sudo docker logs --tail 50 $CONTAINER_NAME'\""
