...
/**
 * @typedef {Object} CreatedReservation
...
/**
 * @typedef {Object} ReservationSearchResult
 * @property {string} appointId
 * @property {string} date
 * @property {string} time
...
/**
 * スクリーンショットを保存
 * @param {string | undefined} base64Data - Base64エンコードされた画像データ
 ...
 * @returns {Promise<{ status: number | undefined; data: any }>} HTTPレスポンス
 */
...
async function runTests() {
  ...
  try {
    ...
    if (success && res.data.reservations?.length > 0) {
      ...
      res.data.reservations.forEach((/** @type {ReservationSearchResult} */ r, /** @type {number} */ i) => {
