...
- `src/` holds the TypeScript service and RPA page objects. Entry point is `src/server.ts`; session/browser utilities live in `src/lib/`; EasyApo page flows live in `src/pages/`.
...
- API requests must include `X-RPA-Login-Id` and `X-RPA-Login-Password` headers.
