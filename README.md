...
| `time` | Yes | 予約時刻（HH:MM） |
| `duration_min` | No | 所要時間（分）。未指定時は自動判定（下記参照） |
...
**新規・既存患者の取り扱い:**
- `customer_id`（患者番号）はSmartCall側では不明のため、指定不要
- `customer_phone`（電話番号）で患者マスタを自動検索し、ヒットすれば既存患者として`patient_id`を自動設定
- 見つからなければ新規患者として予約を作成
...
| 患者が見つからない（新規） | 初診 | 45分 |
| 既存患者で来院履歴なし | 初診 | 45分 |
| 既存患者で前回来院から60日以上経過 | 再初診 | 45分 |
| 既存患者で前回来院から60日未満 | 再診 | 30分 |

**所要時間の優先順位:** `メニューの所要時間` > `duration_min` > `自動判定（再診30分/それ以外45分）` > `45分（デフォルト）`
- `external_menu_id`または`menu_name`でメニューが特定できた場合、そのメニューの所要時間が最優先
- `duration_min`を明示指定した場合は自動判定より優先
- いずれも未指定の場合、電話番号から患者を特定し前回来院日で自動判定
