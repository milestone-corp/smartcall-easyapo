...
| `envs/293-deguchi-shika.env` | でぐち歯科 | 192.168.20.74 | centos | 3011 | 293-deguchi-shika |
...
> でぐち歯科 (293) は当初 RPA04:3013 に同居予定だったが、RPA04 のメモリ枯渇（1.7GB/swap無し）で
> Chromium 起動不能 → サーバフリーズを起こしたため、**単独で RPA05 (`192.168.20.74`):3011 へ移設**した。
> RPA05 は swap 4GB 追加済みで、Chromium が約2秒でナビゲートできることを検証済み。
...
| RPA05 (.74) | 293-deguchi-shika | 3011 | ✅ 稼働中（RPA04から移設、Chromium動作検証済み） |

> **2026-06-30 追記**: でぐち歯科(293)を RPA04→RPA05 へ移設。RPA04 はメモリ1.7GB/swap無しで3院目の
> Chromiumを起動できずフリーズ（PMがRPA02〜05へswap 4GB追加で対応）。RPA04 の旧 293 コンテナは削除済み。
...
| **本番** | 各クリニックの運用 | RPA01 (`192.168.20.70`) / RPA04 (`192.168.20.73`) / RPA05 (`192.168.20.74`) | 店舗別 compose | 店舗別 deploy script |
...
| **293** | でぐち歯科 | `192.168.20.74` | 3011 | `293-deguchi-shika` | [docker-compose.shop.yml](../docker-compose.shop.yml)（統一フロー） | `scripts/deploy.sh 293` | ✅ 稼働中（RPA05・単独） |
...
| RPA05 (`192.168.20.74`) | 踏み台経由 | `centos` | `/home/centos/smartcall-easyapo` |
...
./scripts/deploy.sh 293                                      # でぐち歯科 (RPA05 / 3011)
```
