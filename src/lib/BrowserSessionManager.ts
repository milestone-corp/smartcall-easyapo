 ...
 * ブラウザセッション管理
 *
 * SDKのBaseBrowserSessionManagerを継承し、EasyApo固有のログイン処理を実装
 */
...
/** EasyApo固有のセッション設定 */
export interface EasyApoSessionConfig extends BaseSessionConfig {
  /** ログイン認証情報 */
...
/**
 * EasyApoブラウザセッションマネージャー
 *
 * SDKのBaseBrowserSessionManagerを継承し、EasyApo固有の処理を実装
 */
export class BrowserSessionManager extends BaseBrowserSessionManager {
  private easyApoConfig: Required<EasyApoSessionConfig>;

  constructor(config: EasyApoSessionConfig) {
    // SDKの基底クラスに渡す設定
    super({
    ...
    this.easyApoConfig = {
      ...
      keepAlivePath: config.keepAlivePath ?? '/',
    };
  }
